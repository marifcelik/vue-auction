...
<template>
  <nav class="navbar sm:px-0 md:px-20 border-b-2 border-base-300 bg-base-100 fixed top-0 left-0 z-10">
    <div class="flex-1">
      ...
      <ul class="menu menu-horizontal px-1">
        <li>
          <label class="swap swap-rotate">
            <input type="checkbox" @change="setTheme(store.theme === 'dark' ? 'light' : 'dark')" :checked="store.theme === 'dark'" />
        ...
        <template v-if="store.userId">
          <li>
            <RouterLink :to="{ name: 'products' }">Ürünler</RouterLink>
          </li>
          <li>
            <RouterLink :to="{ name: 'logout' }">Çıkış Yap</RouterLink>
          </li>
        </template>
        <template v-else>
          <li>
